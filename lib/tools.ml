...
let run (k:int) v = 
  ...
  let tk = (time_fun (fun a -> karatsuba (fst a) (snd a)) (p1,p2)) in
...
let export_latex ht (file:string) = 
  ...
  Hashtbl.iter (fun k v -> Printf.fprintf oc "%d %f %f \n" k (snd v).(0) (snd v).(1) ) ht;
  close_out oc;
;;
...
(* let main =
    ...
    let a2 = make_array 11 4000 in (* TODO remplacer par 10000 *)
    shuffle a2;
