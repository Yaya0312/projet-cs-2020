...
(** 
    [get_diag m]
    Retourne la diagonale de la matrice carré m
    Complexity O(n) n étant le nombre de lignes
*)
...
(** 
    [mul_row m row coef]
    Multiplie la ligne row de la matrice m par coeff
    Complexity O(n) TODO
*)
val mul_row : matrix -> int -> float -> unit

(** 
    [sum_row m num_row row]
    Additionne la ligne num_row de la matrice m avec la ligne row
    Complexity O(n) TODO
*)
